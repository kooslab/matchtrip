...
import { fileUploads, products, destinations, users, guideProfiles } from '../src/lib/server/db/schema';
...
async function checkOrphanedFiles() {
	...
	const publicUrls = Array.from(allUrls).filter(url => 
		url.includes('http') || url.includes(process.env.R2_PUBLIC_URL || 'NONE')
	);
