...
async function createBackup() {
  ...
  try {
    ...
    if (stdout.includes('PostgreSQL database dump')) {
      console.log(`${colors.green}✅ Backup file verified - contains PostgreSQL dump${colors.reset}`);
    }
    
    // Count tables in backup
    const { stdout: tableCount } = await execAsync(`grep -c "CREATE TABLE" "${backupFile}" || true`);
    if (tableCount) {
    ...
    console.log(`${colors.yellow}⚠️  Keep this backup safe: ${path.basename(backupFile)}${colors.reset}`);
    ...
    try {
      ...
      console.error(`${colors.red}❌ pg_dump not found. Please install PostgreSQL client tools:${colors.reset}`);
      console.error(`${colors.cyan}   For macOS: brew install postgresql${colors.reset}`);
      console.error(`${colors.cyan}   For Ubuntu: sudo apt-get install postgresql-client${colors.reset}`);
    }
