...
async function checkTable(table: TableCheck) {
	...
	try {
		...
		if (table.expectedPrefixes.length === 1) {
			...
			const conditions = table.expectedPrefixes.map(p => `display_id NOT LIKE '${p}-%'`).join(' AND ');
			prefixCheck = `(${conditions})`;
		}
		
		const wrongPrefix = await db.execute(sql.raw(`
			SELECT display_id, id
			FROM ${table.name}
			WHERE display_id IS NOT NULL
			AND ${prefixCheck}
		...
		console.log(`With display_id: ${withDisplayId} (${total > 0 ? ((withDisplayId/total)*100).toFixed(1) : 0}%)`);
		...
		if (wrongPrefix.rows.length > 0) {
			...
			wrongPrefix.rows.slice(0, 3).forEach((row: any) => {
				console.log(`  - ${row.display_id} (expected prefix: ${table.expectedPrefixes.join(' or ')})`);
			});
		}
