<script lang="ts">
	...
	async function handleSubmit() {
		...
		try {
			...
			if (startDate && typeof startDate === 'object' && 'year' in startDate && 'month' in startDate && 'day' in startDate) {
			...
			if (endDate && typeof endDate === 'object' && 'year' in endDate && 'month' in endDate && 'day' in endDate) {
...
<div class="flex flex-col min-h-screen bg-white">
	...
	<div class="flex-1 flex flex-col items-center justify-center px-4">
		...
		<!-- Success message -->
		<h1 class="text-2xl font-bold text-gray-900 mb-3">여행 등록이 완료되었습니다!</h1>
		<p class="text-gray-600 text-center">
			곧 가이드님들의 제안을 받아보실 수 있어요.
		</p>
		
		<!-- Loading indicator -->
		{#if isSubmitting}
			<div class="flex items-center gap-2 text-gray-500 mt-8">
	...
	<div class="p-4 pb-24">
		<button
			...
			class="w-full py-3 px-4 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
		>
			홈으로 이동
		</button>
	</div>
