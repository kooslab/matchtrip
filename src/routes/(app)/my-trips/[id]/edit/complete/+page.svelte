<script lang="ts">
	...
	let trip = $derived($page.params.id);
	...
	async function handleSubmit() {
		...
		try {
			...
			const tripPayload = {
				destinationId: formData.destinationId || formData.destination?.id,
				startDate,
				endDate,
			...
			console.log('Sending trip update payload:', tripPayload);
			...
			if (!response.ok) {
				...
				throw new Error(result.error || '여행 수정에 실패했습니다.');
			}
			...
			alert(error.message || '여행 수정에 실패했습니다. 다시 시도해주세요.');
		} finally {
			isSubmitting = false;
		}
	}
...
<div class="flex min-h-screen flex-col bg-white">
	...
	<div class="flex flex-1 flex-col items-center justify-center px-4">
		<!-- Complete image -->
		<div class="mb-8">
			<img src={completeImage} alt="여행 수정 완료" class="h-48 w-48 object-contain" />
		</div>

		<!-- Success message -->
		<h1 class="mb-3 text-2xl font-bold text-gray-900">여행 수정이 완료되었습니다!</h1>
		<p class="text-center text-gray-600">변경사항이 저장되었습니다.</p>
