		...
		<div class="mb-4 flex items-center justify-between rounded-lg bg-gray-50 p-3">
			...
			<button
				...
				class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors {
					budgetType === 'flexible' ? 'bg-blue-500' : 'bg-gray-300'
				}"
			>
				<span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform {
					budgetType === 'flexible' ? 'translate-x-6' : 'translate-x-1'
				}"></span>
			</button>
		</div>
			...
			<div class="mb-4 grid grid-cols-2 gap-2">
				{#each budgetRanges as range}
					<button
						...
						class="rounded-lg border py-2 px-3 text-sm font-medium transition-colors {
