...
<button onclick={onclick || (() => {})} class="block w-full text-left">
	...
	<div class="relative aspect-square overflow-hidden rounded-2xl bg-gray-100">
		{#if thumbnailImage}
			<img src={thumbnailImage} alt={product.title} class="h-full w-full object-cover" />
		{:else}
		...
		{#if product.isNew}
			<div class="absolute top-2 left-2">
				<span class="rounded bg-green-500 px-2 py-1 text-xs font-bold text-white">NEW</span>
			</div>
		{:else if product.isHot}
			<div class="absolute top-2 left-2">
				<span class="rounded bg-red-500 px-2 py-1 text-xs font-bold text-white">HOT</span>
			</div>
		{/if}
	</div>
