...
<div class="flex-1 bg-white">
	...
	<div class="px-4 pt-6 pb-4">
		<div class="flex items-start justify-between">
			...
			<button onclick={handleSkip} class="text-sm font-medium text-blue-500"> 건너뛰기 </button>
		</div>
	</div>
	...
	<div class="px-4 pb-6">
		<label class="block">
			...
			/>
			<div
				class="w-full cursor-pointer rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 p-8 transition-colors hover:border-gray-400"
			>
				<div class="text-center">
					<Paperclip class="mx-auto h-12 w-12 text-gray-400" />
					<p class="mt-2 text-sm text-gray-600">파일을 클릭하여 찾아보세요.</p>
					<p class="mt-1 text-xs text-gray-500">
						지원되는 파일 형식: PDF, JPG, JPEG, PNG, DOCX, PPTX, XLSX
					</p>
				</div>
			</div>
		</label>
	...
	{#if uploadedFiles.length > 0}
		<div class="px-4 pb-6">
			<h3 class="mb-3 text-sm font-medium text-gray-700">첨부 파일</h3>
			<div class="space-y-2">
