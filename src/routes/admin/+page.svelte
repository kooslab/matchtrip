...
<div class="p-8">
	...
	<!-- User Statistics -->
	<div class="mb-8">
		<h2 class="text-xl font-semibold text-gray-900 mb-4">사용자 통계</h2>
			...
			<div class="rounded-lg bg-white p-6 shadow">
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-xs text-gray-500 mt-1">인증: {data.stats.verifiedGuides}명</p>
					</div>
	...
	<div class="mb-8 grid gap-8 lg:grid-cols-2">
		<!-- Trip Statistics -->
		<div>
			<h2 class="text-xl font-semibold text-gray-900 mb-4">여행 통계</h2>
			<div class="rounded-lg bg-white p-6 shadow">
				<div class="mb-6">
					...
					<div class="text-xs text-gray-500 mb-1">신규 (30일): {data.stats.newTrips}건</div>
				</div>
				...
				<div class="space-y-3">
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center">
							<FileText class="h-4 w-4 text-blue-400 mr-2" />
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center">
							<CheckCircle class="h-4 w-4 text-green-400 mr-2" />
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center">
							<CheckCircle class="h-4 w-4 text-emerald-400 mr-2" />
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center">
							<XCircle class="h-4 w-4 text-red-400 mr-2" />
		...
		<!-- Offer Statistics -->
		<div>
			<h2 class="text-xl font-semibold text-gray-900 mb-4">오퍼 통계</h2>
			<div class="rounded-lg bg-white p-6 shadow">
				<div class="mb-6">
					<div class="flex items-center justify-between mb-2">
				...
				<div class="space-y-3">
					<div class="flex items-center justify-between">
						<div class="flex items-center">
							<Clock class="h-4 w-4 text-yellow-400 mr-2" />
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center">
							<XCircle class="h-4 w-4 text-gray-400 mr-2" />
							<span class="text-sm text-gray-600">취소됨</span>
						</div>
		...
		<div class="rounded-lg bg-white p-6 shadow">
			<div class="flex items-center justify-between">
				<div>
					...
					<p class="text-2xl font-bold text-gray-900">{formatCurrency(data.stats.payments.totalRevenue)}</p>
					<p class="text-xs text-gray-500 mt-1">성공: {formatCurrency(data.stats.payments.succeeded)}</p>
				</div>
		...
		<div class="rounded-lg bg-white p-6 shadow">
			<div class="flex items-center justify-between">
				<div>
					...
					<p class="text-2xl font-bold text-gray-900">{formatCurrency(data.stats.payments.pending)}</p>
				</div>
							...
							<tr>
								...
								<td class="px-6 py-4 whitespace-nowrap">
									<span class="inline-flex rounded-full px-2 text-xs font-semibold leading-5
										...
										{trip.status === 'draft' ? '초안' :
										 trip.status === 'submitted' ? '제출됨' :
										 trip.status === 'accepted' ? '수락됨' :
										 trip.status === 'completed' ? '완료됨' :
										 trip.status === 'cancelled' ? '취소됨' : trip.status}
									</span>
								</td>
								<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
	...
	<div class="mt-8">
		...
		<div class="grid gap-4 md:grid-cols-2">
			...
			<a href="/admin/users" class="rounded-lg bg-white p-6 shadow hover:shadow-lg transition-shadow">
