...
<div class="min-h-screen bg-gray-50">
	<div class="mx-auto flex min-h-screen max-w-[430px] flex-col bg-white">
		<!-- Header -->
		<div class="flex flex-col">
			<div class="flex items-center px-4 py-3">
				<button
					onclick={() => window.history.back()}
					class="flex h-6 w-6 items-center justify-center text-blue-500"
				>
					{@html IconArrowLeft}
				</button>
			</div>
			<div class="relative">
				...
				<div class="absolute top-0 left-0 h-0.5 w-1/3 bg-blue-500"></div>
			</div>
		</div>
		...
		<div class="flex-1 px-6 py-8">
			...
			<div class="space-y-4">
				...
				<button
					...
					class="relative w-full rounded-2xl border-2 p-6 transition-all {selectedRole ===
					'traveler'
						? 'border-blue-500 bg-gray-50'
						: 'border-gray-200 bg-gray-50'}"
				>
					{#if selectedRole === 'traveler'}
						<div class="absolute top-4 left-4 h-6 w-6 text-blue-500">
							{@html IconCheckCircle}
						</div>
					{:else}
						<div class="absolute top-4 left-4 h-6 w-6 rounded-full border-2 border-gray-300"></div>
					{/if}

					<div class="flex flex-col items-center pt-4">
						<img src={passportImage} alt="여행자" class="mb-4 h-24 w-24 object-contain" />
						<h3 class="mb-2 text-lg font-semibold text-gray-900">여행자</h3>
						<p class="text-center text-sm text-gray-500">
							고민 없이, 나만을 위한 여행 플랜을 받아보세요
						</p>
					</div>
				</button>
				...
				<button
					...
					class="relative w-full rounded-2xl border-2 p-6 transition-all {selectedRole === 'guide'
						? 'border-blue-500 bg-gray-50'
						: 'border-gray-200 bg-gray-50'}"
				>
					...
					<div class="flex flex-col items-center pt-4">
						<img src={scopeImage} alt="가이드" class="mb-4 h-24 w-24 object-contain" />
						<h3 class="mb-2 text-lg font-semibold text-gray-900">가이드</h3>
						<p class="text-center text-sm text-gray-500">
							나의 여행 노하우로 새로운 수익을 만들어보세요
						</p>
					</div>
				</button>
			</div>
