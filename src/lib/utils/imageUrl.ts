 ...
 */
export function transformImageUrl(imageUrl: string | null | undefined, type?: string): string | null {
	...
	if (imageUrl.startsWith('/api/images/')) {
		...
		if (!path.includes('/') && !path.startsWith('destination/') && 
			(path.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\.(jpg|jpeg|png|webp)$/i))) {
