...
export async function canSendVerificationCode(phone: string): Promise<{ allowed: boolean; reason?: string }> {
	...
	const recentCode = await db
		...
		.where(
			and(
				...
				gt(phoneVerifications.createdAt, oneMinuteAgo)
			)
		)
		.limit(1);
	...
	const hourlyCount = await db
		...
		.where(
			and(
				eq(phoneVerifications.phone, phone),
				gt(phoneVerifications.createdAt, oneHourAgo)
			)
		);
		
	if (hourlyCount.length >= MAX_CODES_PER_HOUR) {
		return { allowed: false, reason: '너무 많은 인증 시도가 있었습니다. 잠시 후 다시 시도해주세요.' };
	}
...
export async function cleanupExpiredCodes(): Promise<void> {
	...
	await db
		...
		.where(
			and(
				lt(phoneVerifications.expiresAt, now),
				eq(phoneVerifications.verified, false)
			)
		);
}