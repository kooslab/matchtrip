		...
		if (!selection || selection.rangeCount === 0) {
			...
			if (editorRef) {
				...
				if (isLoading) {
					...
					loadingOverlay.className = 'image-loading-overlay absolute inset-0 flex items-center justify-center';
	...
	<div class="relative">
		...
		{#if showImageButton}
			...
			<button
				...
				class="absolute bottom-3 right-3 flex h-10 w-10 items-center justify-center rounded-full border border-gray-300 bg-white shadow-md transition-all hover:border-gray-400 hover:bg-gray-50"
				title="이미지 추가"
				type="button"
			>
				<img
...
<style>
	...
	.rich-text-editor :global(h1) { font-size: 1.5rem; }
	.rich-text-editor :global(h2) { font-size: 1.25rem; }
	.rich-text-editor :global(h3) { font-size: 1.125rem; }
