...
<Dialog.Root bind:open>
	<Dialog.Portal>
		<Dialog.Overlay class="fixed inset-0 bg-black/60 z-50" style="animation: fadeIn 0.2s ease-out" />
			...
			<div class="flex bg-white rounded-2xl border border-gray-100 shadow-[0_25px_50px_rgba(0,0,0,0.25)] overflow-hidden h-[440px]">
				...
				<div class="w-[80px] bg-[rgba(0,62,129,0.02)] border-r border-[rgba(0,62,129,0.08)] flex flex-col">
					...
					<div class="flex-1 overflow-y-auto py-2 scrollbar-hide">
						{#each Array.from({ length: yearRange }, (_, i) => new Date().getFullYear() - i) as year}
							<button
								type="button"
