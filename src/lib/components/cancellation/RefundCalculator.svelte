...
<div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
	...
	<div class="space-y-2">
		...
		<div class="flex justify-between text-sm">
			...
			<span class="font-medium">{daysBeforeTrip < 0 ? `종료 ${Math.abs(daysBeforeTrip)}일 경과` : `${daysBeforeTrip}일`}</span>
		</div>
		...
		<div class="mt-3 p-2 bg-yellow-50 border border-yellow-200 rounded">
			<div class="flex items-start gap-2">
				...
				<div class="text-xs text-yellow-800">
					...
					<p class="mt-1">
						예외 상황으로 인한 취소는 증빙 서류 검토 후 관리자 승인이 필요합니다.
						승인 시 전액 환불됩니다.
					</p>
				</div>
			</div>
	...
	<div class="mt-3 p-2 bg-blue-50 border border-blue-200 rounded">
		<div class="flex items-start gap-2">
			...
			<p class="text-xs text-blue-800">
				실제 환불 금액은 결제 수단 및 카드사 정책에 따라 약간의 차이가 있을 수 있습니다.
