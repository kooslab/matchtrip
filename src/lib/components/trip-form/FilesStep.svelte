	...
	<div class="px-4 pb-6">
		{#if !selectedFile}
			<label class="block">
				<input
					...
					class="hidden"
				/>
				<div class="w-full p-8 border-2 border-dashed border-gray-300 rounded-lg hover:border-gray-400 transition-colors cursor-pointer">
		...
		{:else}
			...
			<button
				...
				class="mt-3 w-full py-2 px-4 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
			>
				다른 파일 선택
			</button>
		{/if}
